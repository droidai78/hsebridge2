...
async function getItemFromSN(number) {
  ...
  const url =
    `${process.env.SN_INSTANCE}/api/now/table/sc_req_items` +
      ...
      messages: [
        {
          role: 'system',
          content:
            'You are an HSE assistant summarizing health & safety incidents for HSE officers.',
        },
        {
          role: 'user',
          content:
            `Summarize this incident in 5–7 sentences and suggest a risk level and 3 recommended follow-up actions:\n${text}`,
        },
      ],
      temperature: 0.2,
    }),
...
app.post('/hse-summary-incident', async (req, res) => {
  try {
    ...
    const summary = await getIncidentSummaryFromOpenAI(incident);

    res.json({ incident_number, summary });
  } catch (err) {
    console.error('ERROR IN /hse-summary:', err.message);
    res.status(500).json({ error: err.message });
  }
