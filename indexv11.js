...
// ---------- 1) Get item, incident, change from ServiceNow ----------
...
async function getChangeFromSN(number) {
  const pair = `${process.env.SN_USERNAME}:${process.env.SN_PASSWORD}`;
  const auth = Buffer.from(pair, 'ascii').toString('base64');

  const url =
    `${process.env.SN_INSTANCE}/api/now/table/change_request` +
    `?sysparm_query=number=${encodeURIComponent(number)}` +
    `&sysparm_limit=1&sysparm_display_value=true`;
  ...
  const data = JSON.parse(bodyText);
  if (!data.result || data.result.length === 0) {
    throw new Error('Change request not found in ServiceNow');
  }

  return data.result[0];
}

async function getRequestFromSN(number) {
  const pair = `${process.env.SN_USERNAME}:${process.env.SN_PASSWORD}`;
  const auth = Buffer.from(pair, 'ascii').toString('base64');

  const url =
    `${process.env.SN_INSTANCE}/api/now/table/sc_request` +
    `?sysparm_query=number=${encodeURIComponent(number)}` +
    `&sysparm_limit=1&sysparm_display_value=true`;
  ...
  const res = await fetch('https://api.openai.com/v1/chat/completions', {
    ...
    body: JSON.stringify({
      ...
      messages: [
        ...
        **Item Summary:**  
        [Provide a 5–7 sentence summary of the item, including number, short description, priority, and any relevant context. Make it clear whether the incident poses any health, safety, or environmental risks.]
...
async function getRequestSummaryFromOpenAI(request) {
  const text = `
      ...
      messages: [
        {
          role: 'system',
          content:
            'You are an HSE assistant summarizing various requests for HSE officers.',
        },
        {
          role: 'user',
          content: `Please return a JSON object with two fields:
        ...
        1. "summary_and_risk": Format this as follows:
        **Request Summary:**  
        [Provide a 5–7 sentence summary of the request, including number, short description, priority, and any relevant context. Make it clear whether the incident poses any health, safety, or environmental risks.]
...
async function getChangeSummaryFromOpenAI(change) {
  const text = `
  Change Request Number: ${change.number}
      ...
      messages: [
        {
          role: 'system',
          content:
            'You are an HSE assistant summarizing change requests for HSE officers.',
        },
        {
          role: 'user',
          content: `Please return a JSON object with two fields:
        ...
        1. "summary_and_risk": Format this as follows:
        **Change Summary:**  
        [Provide a 5–7 sentence summary of the change, including number, short description, priority, and any relevant context. Make it clear whether the incident poses any health, safety, or environmental risks.]
...
app.post('/hse-summary-item', async (req, res) => {
  try {
    ...
    const { summaryAndRiskLevel, followUpActions } = await getItemSummaryFromOpenAI(item);
    ...
    console.error('ERROR IN /hse-summary-item:', err.message);
...
app.post('/hse-summary-change', async (req, res) => {
  try {
    ...
    console.log('Request for change_number:', change_number);

    const change = await getChangeFromSN(change_number);
    const { summaryAndRiskLevel, followUpActions } = await getChangeSummaryFromOpenAI(change);
    ...
    console.error('ERROR IN /hse-summary-change:', err.message);
    res.status(500).json({ error: err.message });
  }
});

app.post('/hse-summary-request', async (req, res) => {
  try {
    const { req_number } = req.body;
    if (!req_number) {
      return res.status(400).json({ error: 'req_number is required' });
    }

    console.log('Request for req_number:', req_number);

    const request = await getRequestFromSN(req_number);
    const { summaryAndRiskLevel, followUpActions } = await getRequestSummaryFromOpenAI(request);
    ...
    console.error('ERROR IN /hse-summary-request:', err.message);
    res.status(500).json({ error: err.message });
  }
});
