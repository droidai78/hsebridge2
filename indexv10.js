    ...
    body: JSON.stringify({
      ...
      messages: [
        ...
        },
        {
          role: 'user',
          content:
            `Summarize this request in 5–7 sentences and suggest a risk level and 3 recommended follow-up actions:\n${text}`,
        },
      ],
      temperature: 0.2,
    }),
    ...
    body: JSON.stringify({
      ...
      messages: [
        ...
        },
        {
          role: 'user',
          content:
            `Summarize this change request in 5–7 sentences and suggest a risk level and 3 recommended follow-up actions:\n${text}`,
        },
      ],
      temperature: 0.2,
    }),
...
app.post('/hse-summary-change', async (req, res) => {
  try {
    ...
    const summary = await getChangeSummaryFromOpenAI(change);

    res.json({ change_number, summary });
  } catch (err) {
...
app.post('/hse-summary-request', async (req, res) => {
  try {
    ...
    console.log('Request for request_number:', request_number);

    const request = await getRequestFromSN(request_number);
    const summary = await getRequestSummaryFromOpenAI(request);

    res.json({ request_number, summary });
  } catch (err) {
